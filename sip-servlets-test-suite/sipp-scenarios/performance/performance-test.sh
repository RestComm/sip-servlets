...
if [ $# -ne 0 ]; then
	case $1 in	
	    proxy)
				...
				./sipp $TEST_IP:5090 -sf proxy-scenario/performance-proxy-uac.xml -trace_err -i $TEST_IP -p 5055 -rsa $TEST_IP:5080 -r 100 -m 1000000 -nd
				sleep 10
	            ;;
	    b2bua)
	    		rm ./b2bua/*.log
				...
				./sipp $TEST_IP:5080 -s receiver -sf b2bua/call-forwarding-sender.xml -trace_err -i $TEST_IP -p 5050 -r 50 -m 1000000
				sleep 10
	            ;;
	    *)
	    		rm ./uas/*.log
	 			echo "Distributed example used is uas";	    		
	    		./sipp $TEST_IP:5080 -s yousendbye -sf simple-flow/performance-uac.xml -trace_err -i $TEST_IP -p 5055 -r 100 -m 1000000
	            ;;
    esac
fi
