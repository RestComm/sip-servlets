...
if [ "0" = $3 ]; then
  ...
  $JBOSS_HOME/bin/run.sh -Djboss.server.log.threshold=INFO -c $1 &> "jboss-$1-$4.out"&
else
  $JBOSS_HOME/bin/run.sh -Djboss.server.log.threshold=INFO -c $1 -Djboss.service.binding.set=ports-0$3 -Djboss.messaging.ServerPeerID=$3 &> "jboss-$1-$4.out"&
fi
export jbosspid=$!
