...
if [ $# -ne 0 ]; then
	case $1 in	
	    proxy)
				...
				rm -rf $JBOSS_HOME/standalone/$deployments_dir/location-service-distributable-*.war
				cp $EXAMPLES_HOME/location-service-distributable/target/location-service-distributable-*.war $JBOSS_HOME/standalone/$deployments_dir
	    ...
	    b2bua)
				...
				rm -rf $JBOSS_HOME/standalone/$deployments_dir/call-forwarding-distributable-*.war
				cp $EXAMPLES_HOME/call-forwarding-distributable/target/call-forwarding-distributable-*.war $JBOSS_HOME/standalone/$deployments_dir
	    ...
	    custom-b2bua)
				...
				rm -rf $JBOSS_HOME/standalone/$deployments_dir/custom-call-forwarding-distributable-*.war
				cp $EXAMPLES_HOME/custom-call-forwarding-distributable/target/custom-call-forwarding-distributable-*.war $JBOSS_HOME/standalone/$deployments_dir
				cp $EXAMPLES_HOME/custom-call-forwarding-distributable/distributable-call-forwarding-dar.properties $JBOSS_HOME/standalone/configuration/dars/mobicents-dar.properties
	            ;;
	    custom-b2bua-early)
	            # TODO: adapt to eap-6.4
	    ...
	    uas)
				...
				rm -rf $JBOSS_HOME/standalone/$deployments_dir/simple-sip-servlet-distributable-*.war
				cp $EXAMPLES_HOME/simple-sip-servlet-distributable/target/simple-sip-servlet-distributable-*.war $JBOSS_HOME/standalone/$deployments_dir
	    ...
	    uac)
	    		...
	    		rm -rf $JBOSS_HOME/standalone/$deployments_dir/shootist-sip-servlet-distributable-*.war
				cp $EXAMPLES_HOME/shootist-sip-servlet-distributable/target/shootist-sip-servlet-distributable-*.war $JBOSS_HOME/standalone/$deployments_dir
				cp $EXAMPLES_HOME/shootist-sip-servlet-distributable/distributable-shootist-dar.properties $JBOSS_HOME/standalone/configuration/dars/mobicents-dar.properties
	            ;;
	    uac-register)
