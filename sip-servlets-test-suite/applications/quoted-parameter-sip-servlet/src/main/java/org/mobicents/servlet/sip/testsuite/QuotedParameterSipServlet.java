...
public class QuotedParameterSipServlet 
		extends SipServlet 
	...
	private static transient Logger logger = Logger.getLogger(QuotedParameterSipServlet.class);
	...
	/** Creates a new instance of QuotedParameterSipServlet */
	public QuotedParameterSipServlet() {
	}
	...
	protected void doProvisionalResponse(SipServletResponse resp)
		...
		try {
			...
			if(resp.getHeader("require") != null) {
				...
				SipURI requestURI = sipFactory.createSipURI("LittleGuy", "" + System.getProperty("org.mobicents.testsuite.testhostaddr") + ":5080", resp.getSession());
	...
	public void servletInitialized(SipServletContextEvent ce) {
		try {
			...
			URI fromURI = sipFactory.createSipURI(userName, "here.com", sipServletRequest.getSession());
			URI toURI = null;
			if(ce.getServletContext().getInitParameter("urlType") != null) {
				if(ce.getServletContext().getInitParameter("urlType").equalsIgnoreCase("tel")) {
					try {
						toURI = sipFactory.createURI("tel:+358-555-1234567", sipServletRequest.getSession());
					} catch (ServletParseException e) {
					...
					if(ce.getServletContext().getInitParameter("enum") != null) {
						...
						toURI = dnsResolver.getSipURI(toURI, sipServletRequest.getSession());
					}
				} else if(ce.getServletContext().getInitParameter("urlType").equalsIgnoreCase("telAsSip")) {
					try {
						toURI = sipFactory.createURI("sip:+34666777888@192.168.0.20:5080", sipServletRequest.getSession());
					} catch (ServletParseException e) {
						logger.error("Impossible to create the tel URL as SIP", e);
					}
					
					try {
						toURI = sipFactory.createAddress("<sip:+34666777888@192.168.0.20:5080>", sipServletRequest.getSession()).getURI();
					} catch (ServletParseException e) {
						logger.error("Impossible to create the tel URL as SIP", e);
					}
				}
			} else {
				toURI = sipFactory.createSipURI("LittleGuy", "there.com", sipServletRequest.getSession());
			}
			...
			if(routeHeader != null) {
				try {
					sipServletRequest.pushRoute((SipURI) sipFactory.createURI(routeHeader, sipServletRequest.getSession()));
				} catch (ServletParseException e) {
			...
			if(dontSetRURI == null) {
				...
				SipURI requestURI = sipFactory.createSipURI("LittleGuy", host, sipServletRequest.getSession());
