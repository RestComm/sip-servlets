...
public class ProxyBranchTimeoutTest extends SipServletTestCase{
	...
	public void testProxyBranchTimeoutRequestWithoutCancel() throws InterruptedException {
		init();
		try{
			...
			receiverCalls[0].sendIncomingCallResponse(Response.RINGING, "Ringing", 0);
			assertTrue(sender.waitOutgoingCallResponse(5));
			...
			for(int q = 0; q < 5; q++) {
				...
				int responseCode = sender.getLastReceivedResponse().getStatusCode();
				if(responseCode == 408) {code408received = true;break;}
			}
