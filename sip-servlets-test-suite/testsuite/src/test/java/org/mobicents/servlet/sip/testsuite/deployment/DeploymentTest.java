...
/**
 * This test ensures that an application with no appname cannot be deployed
 * successfully into Mobicents Sip Servlets
 *
...
public class DeploymentTest extends SipServletTestCase {
    ...
    public SipStandardContext deployNoAppNameApplication() {
        SipStandardContext ctx = tomcat.deployAppContext(
                projectHome + "/sip-servlets-test-suite/applications/no-app-name-servlet/src/main/sipapp",
                "sip-test-context", "sip-test");
        assertFalse(ctx.getAvailable());
        return ctx;
    }

    public SipStandardContext deployNoMainServletApplication() {
        SipStandardContext ctx = tomcat.deployAppContext(
    ...
    public SipStandardContext deployShootmeApplication(String webContextName, Map<String, String> params) {
    ...
    public SipContext deployShootmeAuthApplication(String webContextName) {
        if (webContextName == null) {
    ...
    public void testBlankSpaceInDarPath() throws Exception {
        ...
        SipContext sipContext = deployShootmeApplication(null, params);
    ...
    /**
     * Issue 1411 http://code.google.com/p/mobicents/issues/detail?id=1411 Sip
     * Connectors should be removed after removing all Sip Servlets
     *
     * @throws Exception
     */
    public void testBYEOnSipServletDestroy() throws Exception {
        ...
        int myPort = NetworkPortAssigner.retrieveNextPort();
        sender = new TestSipListener(myPort, containerPort, senderProtocolObjects, false);
        SipProvider senderProvider = sender.createProvider();
        senderProvider.addSipListener(sender);
        senderProtocolObjects.start();
    ...
    public void testDeploy2AppsWithSameAppName() throws Exception {
        tomcat.startTomcat();
        deployShootmeApplication(null, null);
        try {
            deployShootmeApplication("second-test-app", null);
