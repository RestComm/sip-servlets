...
The &PLATFORM_NAME;SIP Servlets Server can run on either the JBoss Application Server or the Tomcat Servlet Container.
...
.Differences Between a Standard JBoss Installation and the &PLATFORM_NAME; SIP Servlets Version
...
Sufficient Disk Space::
  Once unzipped, version &VERSION;of the &SHORT_PLATFORM_NAME;for JBoss binary release requires a minimum of &SSS_MSS4J_SIZE;free disk space.

Anything Java Itself Will Run On::
  &SHORT_PLATFORM_NAME;for JBoss is 100% Java and will run on the same hardware that the JBoss Application Server runs on.

.Software Prerequisites
JDK 5 or Higher::
  A working installation of the Java Development Kit (<acronym>JDK</acronym>) version 5 or higher is currently required in order to run &SHORT_PLATFORM_NAME;for JBoss binary distribution.
...
The latest version of &PLATFORM_NAME;SIP Servlets for JBoss is available from &DOWNLOAD_LINK;link:.

Each version of the SIP Servlets Server is comprised of tree separate binary distribution files: one which is &SHORT_PLATFORM_NAME;for JBoss, one which is &SHORT_PLATFORM_NAME;for EAP and the other which is &SHORT_PLATFORM_NAME;for Tomcat.
...
Once the requirements and prerequisites have been met and you have downloaded the binary distribution zip file, you are ready to install the &SHORT_PLATFORM_NAME;for JBoss binary distribution.
...
.Procedure: Installing the &SHORT_PLATFORM_NAME; for JBoss Binary Distribution on Linux
...
. Create a subdirectory to extract the &SHORT_PLATFORM_NAME;for JBoss files into.
...
.Procedure: Installing the &SHORT_PLATFORM_NAME; for JBoss Binary Distribution on 
...
. It is recommended that the folder holding the &SHORT_PLATFORM_NAME;for JBoss files (in this example, the folder named [path]_-jboss-<version>_) is moved to a user-defined location for storing executable programs.
...
Configuring &SHORT_PLATFORM_NAME;for JBoss consists of setting the [var]`JBOSS_HOME` environment variable and optionally customizing the &SHORT_PLATFORM_NAME; for JBoss server by adding SIP Connectors, configuring the application router, and logging.

After setting [var]`JBOSS_HOME` according to the instructions in the following section, refer to <<_bsssc_binary_sip_servlets_server_configuring>> to learn how to configure &SHORT_PLATFORM_NAME; for JBoss.

Alternatively, after having set [var]`JBOSS_HOME`, the &SHORT_PLATFORM_NAME; for JBoss server can be run.
...
To configure &SHORT_PLATFORM_NAME;for JBoss, refer to <<_bsssc_binary_sip_servlets_server_configuring>>.
...
.Procedure: Running &SHORT_PLATFORM_NAME; for JBoss on Linux
. Change the working directory to &SHORT_PLATFORM_NAME;for JBoss's installation directory (the one in which the zip file's contents was extracted to)
+
----
...
.Procedure: Running &SHORT_PLATFORM_NAME; for JBoss on 

There are several ways to start &SHORT_PLATFORM_NAME;for JBoss on Windows.
...
. The preferred way to start &SHORT_PLATFORM_NAME;for JBoss from the Command Prompt.
...
.Procedure: Stopping &SHORT_PLATFORM_NAME; for JBoss on Linux
...
. Stopping the JBoss Application Server on Windows consists in executing either the [path]_shutdown.bat_ or the [path]_shutdown.jar_ executable file in the [path]_bin_ subdirectory of the &SHORT_PLATFORM_NAME; for JBoss binary distribution.
...
To uninstall &SHORT_PLATFORM_NAME;for JBoss, delete the directory containing the binary distribution.