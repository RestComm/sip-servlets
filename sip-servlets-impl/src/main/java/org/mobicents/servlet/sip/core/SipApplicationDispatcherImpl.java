	...
	public void processRequest(RequestEvent requestEvent) {
		...
		try {
			...
			if(requestRouteHeader == null && requestURI != null && 
				...
				// checking if the application name is not null. Misbehaving app can send a request with a no route header
				// and a to tag that is not generated by our container 
				if(ar.getApplicationName() != null) {
					javax.sip.address.SipURI localUri = JainSipUtils.createRecordRouteURI(
							...
							request);
					if(arText != null) {
						localUri.setParameter(MessageDispatcher.RR_PARAM_APPLICATION_NAME, getHashFromApplicationName(ar.getApplicationName()));
						if(ar.getAppGeneratedApplicationSessionId() != null && ar.getAppGeneratedApplicationSessionId().length() > 1) {
							localUri.setParameter(MessageDispatcher.APP_ID, ar.getAppGeneratedApplicationSessionId());
						}
